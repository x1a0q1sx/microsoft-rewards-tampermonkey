# Microsoft Rewards Tampermonkey 助手

一个用于 Microsoft Rewards 的 Tampermonkey 用户脚本，提供搜索、活动、每日连续打卡和阅读任务的辅助执行功能。

> 本项目为个人学习与自动化研究项目。Microsoft Rewards 的规则和接口可能随时变化，请自行评估使用风险，并遵守 Microsoft 服务条款。

## 功能

- PC 搜索和移动搜索任务
- Rewards 活动任务辅助执行
- “每日连续打卡活动”侧边栏及子活动卡片点击
- 阅读任务
- 活动跨页面恢复
- 活动标签页完成后自动关闭
- 活动失败最多重试 3 次
- 搜索达到设定次数后自动暂停 1 分钟
- 悬浮窗显示关键运行日志

## 安装

1. 安装 [Tampermonkey](https://www.tampermonkey.net/)。
2. 打开 Tampermonkey 管理面板。
3. 新建脚本，或直接将 `Get_Microsoft_Rewards_fixed.user.js` 拖入浏览器。
4. 保存并启用脚本。
5. 打开 [Microsoft Rewards](https://rewards.bing.com/)，确认右侧出现脚本悬浮窗。

也可以直接打开脚本文件，选择使用 Tampermonkey 安装。

已经在用旧版本时，建议直接覆盖安装，别只靠“检查更新”：

```text
https://raw.githubusercontent.com/x1a0q1sx/microsoft-rewards-tampermonkey/main/Get_Microsoft_Rewards_fixed.user.js
```

装完后按 F12 打开控制台，第一条日志里的版本号就是当前生效的版本（例如 `v1.0.3.2`）。如果版本号还是旧的，说明浏览器加载的是缓存副本，需要重新导入。

## 使用

登录 Microsoft Rewards 后，在 Rewards 页面使用悬浮窗中的功能按钮：

- “搜索”：执行搜索任务。
- “活动”：执行可识别的活动任务。
- “一键全部执行”：按脚本流程执行搜索、活动及其他任务。

首次使用建议先手动观察一次运行过程，确认账号地区、任务类型和浏览器行为符合预期。

## 定时自动执行

油猴脚本不能脱离浏览器直接运行。若需要用青龙面板、Windows 任务计划程序或系统定时器触发，建议让定时任务启动 Edge 并打开：

```text
https://rewards.bing.com/?mr_auto_run=1
```

脚本检测到 `mr_auto_run=1` 后，会自动点击悬浮窗中的“一键全部执行”。浏览器窗口需要保持打开，脚本会在页面内继续执行。

青龙面板相关文件与教程位于：[qinglong/README.md](qinglong/README.md)。

如果 Edge 打开 Rewards 页面时下载 `Clear.PNG`，通常是下载类扩展拦截了微软埋点资源。优先在 `edge://extensions` 中临时禁用“迅雷下载支持”等下载扩展；这类扩展可能会干扰页面脚本注入。

## 配置

脚本中的配置位于 `CONFIG` 对象，例如：

```js
pause: {
    enabled: true,
    interval: 10,
    duration: 60 * 1000
}
```

当前默认设置为每完成 10 次搜索后暂停 1 分钟。活动失败时最多尝试 3 次（首次执行加 2 次重试）。

## 搜索额度说明

`PC x/y` 和 `移动 x/y` 不是固定值，Microsoft 会根据账号地区、浏览器客户端、登录态和每日完成情况返回不同计数器。

- PC 搜索：Edge 通常能看到浏览器侧 `PCSearch` 计数器；Chrome 不一定有同样额度。脚本会先尝试 Rewards API 和 Bing Flyout，仍拿不到计数器时会显示“无搜索额度”。
- 移动搜索：多数情况下需要 Bing 移动端 App 或真实移动端场景，桌面浏览器不能可靠完成。
- 每天 0 点后额度重置。如果之前有、现在没有，先看是否已经完成，或刷新 Rewards 仪表板确认当日进度。
- 新版脚本会输出一行 `搜索额度：PC ... | 移动 ... | 来源 ...`，可用于判断是微软没返回，还是浏览器/地区限制。

## 常见问题

如果提示 `invalid_grant: The provided value for the 'code' parameter is not valid`，说明这次授权码已经失效、已被消费，或旧版本把它重复编码了。请先升级到最新版本，然后在悬浮窗里重新点击「获取授权码」完成一次新授权。旧授权码是一次性的，不能刷新或重复使用。

如果出现"页面空白等待 → 提示需要授权码 → 页面刷新 → 再次循环"，请确认版本不低于 `1.1.0.0`。1.1.0.0 之前的版本在 OAuth 回调页与已打开的 Rewards 页之间会**并发兑换同一个一次性授权码**：只有第一个成功，其余必然 `invalid_grant`，于是反复要求重新授权。新版改为回调页只把授权码交接给主页面、由单一页面独占兑换；并且已登录（cookie 会话可用）时不再提示授权。

悬浮窗完全不出现，通常是脚本在页面里抛异常提前中断了。先按 F12 看控制台有没有 `Microsoft Rewards 助手 vX 已就绪`：有横幅但没有面板，就把同一条命令的报错发给 Issue；连横幅都没有，一般是版本没更新成功或 Tampermonkey 未在该站点启用。1.0.3.2 修掉了一类典型原因：清理无用代码时误删了几个存储键常量，脚本一执行到活动流程就抛 `ReferenceError`，面板也就建不出来了。

## 调试日志（排查"页面自己刷新/跳转"）

`1.1.1.0` 起内置跨刷新审计层。之前面板日志只存在 DOM 里，页面一刷新就全丢，"第二个活动执行后界面刷新"这类问题无从定位；现在三类证据写入 Tampermonkey 存储（所有标签页共享，刷新不丢）：

- **持久日志缓冲**（约 400 条，含默认不显示的细节日志）：页面加载时自动把**上一页**的日志回放进面板（蓝色虚线分隔），刷新后立刻能看到刷新前发生了什么。
- **导航意图**：脚本每次主动跳转前先落盘"谁为什么跳"。新页面加载时若 90 秒内有跳转记录，面板会红字播报 `🚨 上次跳转由脚本发起：「原因」`。
- **离页快照 + 末次点击**：页面离开（刷新/跳转/关闭）瞬间记录当时 URL 与最后点击的元素。若离开并非脚本导航点触发，面板会播报 `🚨 上次页面离开未经脚本导航点触发`，并附上一页**末次点击**的元素与链接——用来区分"脚本跳的"还是"页面自己跳的"。

排查方法：复现一次问题 → 看面板红字结论与上一页日志；需要完整细节时，在 Tampermonkey 菜单点「🧾 导出调试日志(控制台)」，F12 控制台会输出全部缓冲（含每个标签页交织的时间线、导航意图、离页快照、末次点击）。把输出贴给 Issue 时请先抹掉个人信息。

### 会话失效保护（1.1.2.0）

如果日志里反复出现「上次页面离开未经脚本导航点」且落地点是 `login.live.com/oauth20_authorize.srf?client_id=9c941f7c…`（注意：这是**微软 Rewards 网页自己**的授权，不是脚本的授权），说明 `rewards.bing.com` 的登录会话已失效，而站点会在你点击任何活动卡片时把整页弹去重新登录。此时脚本会：

- 检测到接口 401 后明确提示「rewards.bing.com 会话已失效」，并**暂停所有页面点击类活动**（点击必然被站点弹走，做了也白做）；
- 若短时间内已发生 2 次「非脚本弹跳」，**熔断自动恢复**，不再自动重跑活动，避免「弹走 → 恢复 → 再点 → 再弹」的死循环。

遇到这个提示时，正确做法是：在 `rewards.bing.com` 重新登录一次（或点「🔗 获取授权码」走 OAuth），然后手动点「活动」继续。若登录后仍反复弹跳，按顺序检查：Edge 登录的微软账号与 Rewards 账号是否一致、是否阻止了第三方 Cookie、跟踪防护是否过严，以及代理是否影响 `login.live.com`。

## 注意事项

- 脚本依赖 Microsoft Rewards 页面结构和接口，页面改版后可能需要调整。
- 不要在多个浏览器标签页同时执行同一批任务，以免任务状态互相覆盖。
- 自动打开的活动标签页会在完成后尝试关闭；如果浏览器阻止关闭，请手动关闭。
- 网络异常、登录状态失效、地区限制或任务本身不可用，都可能导致活动失败。
- 不要将账号 Cookie、访问令牌、日志截图或个人信息提交到公开仓库。

## 隐私说明

脚本只在匹配的 Bing、Microsoft Rewards 和登录页面运行，并通过浏览器当前登录状态访问 Rewards 相关页面和接口。项目不提供账号托管，也不应收集或上传账号凭据。

发布到 GitHub 前，请检查代码、Issue、截图和提交历史，确认其中没有 Cookie、Token、账号标识或其他敏感信息。

## 本地自检（贡献者）

改完脚本后，推送前先跑两个零副作用的检查：

```bash
node tools/check-script.js          # 语法 + 未声明标识符扫描 + 无头执行顶层与按钮回调
node tools/test-auth-handoff.js     # 授权交接回归（防授权码死循环）
node tools/test-session-guard.js    # 会话失效保护回归（防站点登录弹跳死循环）
python tools/preview-panel.py       # 用本机 Edge 渲染悬浮窗，输出截图与控制台报错
```

`check-script.js` 会把面板按钮的点击回调也跑一遍，任何 `ReferenceError`/`TypeError` 都会直接失败退出（`exit 1`），适合挂进 CI。静态扫描依赖 `acorn`，没装会自动跳过：

```bash
npm i --prefix tools acorn
```

`preview-panel.py` 会拦截网络、只喂一个 mock 的 `rewards.bing.com/earn` 页面，因此不需要登录就能确认面板 DOM、CSS 和按钮都在；截图写到系统临时目录，控制台同时打印面板尺寸与可见按钮列表。

`test-auth-handoff.js` 用共享桩 `tools/harness.js` 同时模拟"OAuth 回调页"和"另一个已打开的 Rewards 页"，断言同一个授权码最多只被兑换一次、回调页自己不兑换、无 nonce 的第三方回调被忽略、失效 code 不会引发刷新循环。

## 免责声明

本项目按“现状”提供，不保证持续可用，也不对因使用脚本造成的账号限制、积分异常、数据丢失或其他损失负责。使用者应自行承担使用后果，并遵守适用的法律法规及相关服务条款。

## 开源协议

建议在发布时一并添加 `LICENSE` 文件。若你希望采用宽松许可，可以选择 MIT License；正式发布前请根据你的实际需求确认许可证内容。
